...
def gconnect():
	...
	if result['issued_to'] != CLIENT_ID:
		response = make_response(json.dumps("Token's client id does not match app id"), 401)
...
def newItem():
	...
	if request.method == 'POST':
		item = session.query(Items).filter_by(name = request.form['name'], category_id = request.form['category']).first()
		if item == None:
			newItem = Items(name = request.form['name'], description = request.form['description'], category_id = request.form['category'],
			...
			flash("An item with that same name already exists in that category!")
			categories = session.query(Categories).all()
...
def newCat():
	if 'username' not in login_session:
		...
		return redirect(url_for('showLogin'))
	if request.method == 'POST':
		catDupe = session.query(Categories).filter_by(name=request.form['name']).first()
		if catDupe == None:
			newCat = Categories(name = request.form['name'], user_name = login_session['username'])
			session.add(newCat)
			session.commit()
			flash("New item has been added")
			return redirect(url_for('home'))
...
def categoryDisplay(category_name):
	...
	return render_template("categorydisplay.html", categories = categories, category = category, items = items, login_session = login_session)
...
def itemDisplay(category_name, item_name):
	...
	return render_template("itemdisplay.html", item = item, login_session = login_session)
...
def editItem(category_name, item_name):
	...
	if request.method == 'POST':
		dupeItem = session.query(Items).filter(Items.name == request.form['name']).filter(Items.id != item.id).first()
		if dupeItem == None:
			...
			return redirect(url_for('itemDisplay', category_name = item.getCatName, item_name = item.name))
		else:
			flash("An item with that same name already exists in this category!")
